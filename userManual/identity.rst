...
To manipulate resources, people need to create an account that is the root identity to own all their resources. There
are two types of account: admin and normal. Admin accounts, which have unlimited permissions, are owned by administrators.
...
A list of admin-only APIs can be found at :ref:`admin-only APIs <admin-only APIs>`, and a list of non-admin APIs can be
found at :ref:`non-admin APIs <non-admin APIs>`
...
permission control. Users can only perform APIs assigned by :ref:`policy <policy>`.
...
Accounts can create groups and add users into them. By assigning policies to groups, accounts can grant the same permissions
to a group of users.
...
Polices are permissions that define what APIs users can perform. A policy consists of an array of statements each of which
...
.. list-table::
   ...
   * - **accountUuid**
     ...
     - 0.6
   * - **statements**
     - a list of :ref:`statement <statement>` defining API permissions
     -
     -
     - 0.6
   * - **createDate**
...
A statement is a JSON text, containing a list of API identity and an effect: *Allow* or *Deny*. A statement looks like:
...
or a regular expression that can match multiple APIs, for example, *instance:.** will match all APIs under the category *instance*.
...
.. note:: In this version, all *read* APIs have an extra identity in format of *api_category:read*. The read APIs are those not performing
          operations but getting information from ZStack. For example, all query APIs are read API; for API APIQueryVmInstanceMsg, it has
          the default identity *instance:APIQueryVmInstanceMsg* and an extra identity *instance:read*.
...
ZStack automatically assigns default quotas to it, and admins can change them by API *UpdateQuota*. A list of default quotas
...
This most exciting thing of identity system is that you can control API permissions, deciding what people can perform what
APIs. When people login into ZStack, depending on the way they login, they can get different permissions regarding APIs.

**Administrators**: When login using an admin account, the people can perform any APIs.

**Non-admin Account**: When login using a non-admin account, the people can perform any non-admin APIs.
...
in a team that needs to apply some virtual machines in your company's IT infrastructure managed by ZStack. The first
...
In this organization, you have an infrastructure group responsible for managing VMs of your team; the group has three members:
...
multiple polices attached either from themselves or groups they are in. When users call APIs, it always evaluate
...
After installing ZStack, an admin account(account name: admin, password: password) is created by default. Admins can
...
.. note:: In this version as the concept *role* has not been supported, shared resources can only be read by other accounts.
          That is to say, other accounts can query shared resources and use them (e.g. use images to create VMs) but cannot
          perform operations on the shared resources, for example, other accounts cannot delete a shared image.
...
.. list-table::
   ...
   * - **name**
     ...
     - 0.6
   * - **statements**
     - a JSON string representing :ref:`statement <statement>`
     -
     -
     - 0.6
...
.. list-table::
   ...
   * - **password**
     ...
     - 0.6
   * - **uuid**
     - the uuid of account to reset the password. It's mainly used by the admin account to reset password of other
       accounts. For non-admin account, this field is ignored as ZStack can figure out the account uuid by the
       current session.
     - true
     -
     - 0.6
...
.. list-table::
   ...
   * - **password**
     ...
     - 0.6
   * - **uuid**
     - the user uuid. It's mainly used by the account to change the password of the user. For user changing own
